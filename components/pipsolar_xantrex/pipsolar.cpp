...
void Pipsolar::loop() {
  ...
  if (this->state_ == STATE_POLL_CHECKED) {
    bool enabled = true;
    std::string fc;
    char const *tmp = reinterpret_cast<char const *>(this->read_buffer_.data());
    switch (this->used_polling_commands_[this->last_polling_command_].identifier) {
      case POLLING_QPIRI:
  ...
  if (this->state_ == STATE_POLL_COMPLETE) {
    if (this->check_incoming_crc_()) {
      ...
      ESP_LOGW(TAG, "POLL response had invalid CRC");
      this->state_ = STATE_IDLE;
    }
  }
  ...
  if (this->state_ == STATE_COMMAND || this->state_ == STATE_POLL) {
    while (this->available()) {
      ...
      if (this->read_buffer_.size() == PIPSOLAR_READ_BUFFER_LENGTH) {
        this->read_buffer_.clear();
        this->empty_uart_buffer_();
        ESP_LOGW(TAG, "Discarded buffer contents");
      }
      this->read_buffer_.push_back(byte);

      // end of answer
      if (byte == 0x0D) {
        ESP_LOGD(TAG, "Got end-of-line after %u bytes", this->read_buffer_.size());
        this->empty_uart_buffer_();
  ...
  if (this->state_ == STATE_POLL) {
    if (millis() - this->command_start_millis_ > esphome::pipsolar::Pipsolar::COMMAND_TIMEOUT) {
      // command timeout
      ESP_LOGD(TAG, "timeout command to poll: %s (sent %u, now %u)", this->used_polling_commands_[this->last_polling_command_].command,
              this->command_start_millis_,
              millis());
      this->state_ = STATE_IDLE;
    } else {
    }
...
bool Pipsolar::check_incoming_length_(uint8_t length) {
  return this->read_buffer_.size() == static_cast<size_t>(length) + 3;
}

// Given a message of N+3 bytes, validates that the first N bytes have the CRC
// described in bytes N,N+1.
//
// Requires that the length of the buffer has already been checked, otherwise
// this will read off the end.
//
// On success, returns 'true' and ensures that the read buffer is
// NUL-terminated.
//
// If the CRC does not match, returns 'false,' and the read buffer is not
// NUL-terminated.
bool Pipsolar::check_incoming_crc_() {
  uint16_t crc16;
  ESP_LOGD(TAG, "checking crc on incoming message");
  auto const pos = this->read_buffer_.size();
  crc16 = this->pipsolar_crc_(&this->read_buffer_[0], pos - 3);
  if (uint8_t(crc16 >> 8) == read_buffer_[pos - 3] &&
      uint8_t(crc16) == read_buffer_[pos - 2]) {
    ESP_LOGD(TAG, "CRC OK");
    read_buffer_[pos - 1] = 0;
    read_buffer_[pos - 2] = 0;
    read_buffer_[pos - 3] = 0;
    return true;
  }
  ESP_LOGD(TAG, "CRC NOK expected: %X %X but got: %X %X",
          uint8_t(crc16 >> 8), uint8_t(crc16),
          read_buffer_[pos - 3], read_buffer_[pos - 2]);
  return false;
}
...
uint8_t Pipsolar::send_next_command_() {
  ...
  if (!this->command_queue_[this->command_queue_position_].empty()) {
    ...
    // end Byte
    this->write(0x0D);
    ESP_LOGD(TAG, "Sent command from queue: %s with length %d (t=%u)", command, length, this->command_start_millis_);
    return 1;
  }
  return 0;
}
...
void Pipsolar::send_next_poll_() {
  ...
  // end Byte
  this->write(0x0D);
  ESP_LOGD(TAG, "Sent polling command : %s with length %d @%u",
